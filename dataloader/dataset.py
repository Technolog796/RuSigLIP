...
class RuSigLIPDataset(Dataset):
    def __init__(
        ...
        self.images_ids, self.labels_en, self.labels_ru = self._get_images_and_labels(dataset_directory + "data.json")

        if load_tokenized_files:
            self.tokenized_labels_en = torch.load(dataset_directory + "tokenized_labels_en.pt")
            self.tokenized_labels_ru = torch.load(dataset_directory + "tokenized_labels_ru.pt")
            ...
            self.tokenized_labels_en = {key: torch.tensor(value) for key, value in tokenized_labels_en.items()}
            self.tokenized_labels_ru = {key: torch.tensor(value) for key, value in tokenized_labels_ru.items()}

            if save_tokenized_files:
                torch.save(self.tokenized_labels_en, dataset_directory + "tokenized_labels_en.pt")
                torch.save(self.tokenized_labels_ru, dataset_directory + "tokenized_labels_ru.pt")
    ...
    def get_texts(self, indices: Tensor) -> dict[str, dict[str, Tensor]]:
        return {"en": {"input_ids": self.tokenized_labels_en["input_ids"][indices],
